...
function signalApi(){return window.WardenM17SignalAudio||null;}
...
function scheduleLight(){
  clearLightTimer();if(!lightActive)return;
  lightTimer=setTimeout(()=>{if(!lightActive)return;playRecorded(LIGHT_URL,.92,lightPlaying,null);scheduleLight();},rand(300000,600000));
}
function scheduleRocks(first=false){
  clearRockTimer();if(!rocksActive)return;
  rocksTimer=setTimeout(()=>{if(!rocksActive)return;playRecorded(ROCK_URL,.95,rockPlaying,null);scheduleRocks(false);},first?rand(7000,16000):rand(22000,48000));
}

function answerActive(){const s=signalApi();return !!(s&&s.isActive&&s.isActive('answer'));}
function syncOwnButtons(){
  ...
  const ans=document.querySelector('[data-group1-answer-layer]');if(ans){const on=answerActive();ans.classList.toggle('active',on);ans.setAttribute('aria-pressed',String(on));const s=ans.querySelector('.wa-state');if(s)s.textContent=on?'ON':'OFF';}
}
function syncLive(){
  ...
  if(answerActive()&&!names.includes('ANSWERING PULSE'))names.push('ANSWERING PULSE');
  live.textContent=names.length?names.join(' + '):'SILENT';
  syncOwnButtons();
}

function setLight(on){
  on=!!on;if(lightActive===on){syncLive();return;}lightActive=on;clearLightTimer();
  if(on){playRecorded(LIGHT_URL,.92,lightPlaying,null);scheduleLight();}else stopSet(lightPlaying);syncLive();
}
function setRocks(on){
  on=!!on;if(rocksActive===on){syncLive();return;}rocksActive=on;clearRockTimer();
  if(on)scheduleRocks(true);else stopSet(rockPlaying);syncLive();
}
function setAnswer(on){const s=signalApi();if(s&&s.setLayer)s.setLayer('answer',!!on);syncLive();}
...
function triggerAnswer(){const s=signalApi();if(s&&s.trigger)s.trigger('answer');}

function setLayer(id,on){
  if(id==='light')return setLight(on);if(id==='rocks')return setRocks(on);if(id==='answer')return setAnswer(on);if(id==='metal')return;
  const out=baseSetLayer&&baseSetLayer(id,on);syncLive();return out;
}
function trigger(id){
  if(id==='light')return triggerLight();if(id==='rocks')return triggerRocks();if(id==='answer')return triggerAnswer();if(id==='metal')return;
  return baseTrigger&&baseTrigger(id);
}
function startM17(){
  BASE_IDS.forEach(id=>baseSetLayer&&baseSetLayer(id,true));
  setLight(true);setRocks(true);setAnswer(true);syncLive();
}
function stopAll(){
  BASE_IDS.forEach(id=>{try{baseSetLayer&&baseSetLayer(id,false);}catch(_){ }});
  setLight(false);setRocks(false);setAnswer(false);syncLive();
}
...
function install(){
  ...
  const body=document.querySelector('#wardenAudioDock .wa-body');
  const grid=body&&body.querySelector('.wa-grid');
  if(body&&!document.getElementById('waGroup1Header')){
    const header=document.createElement('div');header.id='waGroup1Header';header.className='wa-live';header.style.marginTop='8px';header.innerHTML='<span>GROUP 1 // M-17 + RESPONSE:</span> <b>ENVIRONMENT</b>';
    body.insertBefore(header,grid||null);
  }
  ...
  BASE_IDS.forEach(id=>{
    const oldBtn=document.querySelector(`[data-audio-layer="${id}"]`);if(!oldBtn)return;
    const fresh=replaceButton(oldBtn,()=>{const next=fresh.getAttribute('aria-pressed')!=='true';baseSetLayer&&baseSetLayer(id,next);syncLive();});
  });

  if(grid&&!document.querySelector('[data-group1-answer-layer]')){
    const row=document.createElement('div');row.className='wa-layer';
    row.innerHTML='<button class="wa-layer-toggle" type="button" data-group1-answer-layer aria-pressed="false"><span><b>ANSWERING PULSE</b><small>Distant recorded response — Group 1 volume; cadence accelerates as CROSSFADE / DEPTH increases</small></span><span class="wa-state">OFF</span></button><button class="wa-mini" type="button" data-group1-answer-trigger>TRIGGER</button>';
    grid.appendChild(row);
    row.querySelector('[data-group1-answer-layer]').addEventListener('click',()=>setAnswer(!answerActive()));
    row.querySelector('[data-group1-answer-trigger]').addEventListener('click',triggerAnswer);
  }
  ...
  installed=true;syncLive();try{sig.render&&sig.render();}catch(_){ }return true;
}
