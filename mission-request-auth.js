(() => {
  ...
  function ensureHeaderStatus() {
    ...
    status.innerHTML = `
      <span style="color:#bdd2aa;font-weight:800;letter-spacing:.06em">MISSION REQUEST // AUTHENTICATED</span>
    ...
    status.querySelector('#' + LOCK_ID)?.addEventListener('click', clearSession);
    return status;
  }
  ...
  function ensureGate() {
    ...
    gate.innerHTML = `
      <div role="dialog" aria-modal="true" aria-label="Mission request authentication" style="width:min(520px,100%);background:#111315;border:1px solid #3a3f43;border-radius:5px;box-shadow:0 24px 60px rgba(0,0,0,.55);padding:22px">
        <div style="font-size:1.12rem;font-weight:800;letter-spacing:.08em">MISSION REQUEST AUTHORIZATION</div>
        <div style="color:#aaa79f;font-size:.78rem;margin-top:3px">THE HUB // CONTRACTING SYSTEM</div>
        <div style="margin-top:20px;color:#aaa79f;font-size:.72rem;letter-spacing:.08em">REQUEST CODE</div>
        <input id="boardAccessGatePin" type="password" autocomplete="off" placeholder="Enter request code" style="width:100%;margin-top:7px;padding:11px;font:inherit;color:#e7e4dc;background:#202428;border:1px solid #3a3f43;border-radius:3px">
        ...
        <div style="display:flex;gap:9px;flex-wrap:wrap;margin-top:18px">
          <button id="boardAccessGateSubmit" type="button" style="font:inherit;font-weight:800;letter-spacing:.06em;cursor:pointer;padding:10px 14px;border-radius:3px;border:1px solid #d4a84b;color:#e7e4dc;background:rgba(212,168,75,.12)">AUTHORIZE REQUEST</button>
          <button id="boardAccessGateCancel" type="button" style="font:inherit;font-weight:800;letter-spacing:.06em;cursor:pointer;padding:10px 14px;border-radius:3px;border:1px solid #3a3f43;color:#e7e4dc;background:transparent">CANCEL REQUEST</button>
        </div>
      </div>
    `;
  ...
  function syncUi() {
    ...
    if (status) status.classList.toggle('hidden', !session || !missionsActive());
  }
  ...
  function clearSession() {
    ...
    const pin = document.getElementById('acceptPin');
    if (pin) pin.value = '';
    ensureHeaderStatus()?.classList.add('hidden');
    hideGate();
  }
  ...
  function setGateBusy(busy) {
    ...
    if (submit) {
      ...
      submit.textContent = busy ? 'AUTHORIZING…' : 'AUTHORIZE REQUEST';
    }
  ...
  function exchangePinForSession(pin) {
    ...
    function fail(message) {
      cleanup();
      showGate(message || 'REQUEST CODE DENIED');
    ...
    window[callbackName] = payload => {
      ...
      if (!valid) {
        fail(payload?.message || payload?.error || 'REQUEST CODE DENIED');
        return;
      }
      cleanup();
    ...
    script.onerror = () => fail('MISSION REQUEST AUTHENTICATION SERVICE UNAVAILABLE');
    ...
    timeout = setTimeout(() => fail('MISSION REQUEST AUTHENTICATION TIMED OUT'), 8000);
  ...
  function wrapPrivilegedJsonp(script) {
    ...
    const wrapped = payload => {
      ...
      if (authenticationError(payload)) {
        clearSession();
        setTimeout(() => showGate('SESSION EXPIRED // ENTER REQUEST CODE'), 0);
      }
    };
  ...
  function installJsonpInterceptor() {
    ...
    if (previousAppendChild.__missionRequestAuthWrapped) return;
    function patchedAppendChild(node) {
      wrapPrivilegedJsonp(node);
      return previousAppendChild.call(this, node);
    }
    patchedAppendChild.__missionRequestAuthWrapped = true;
