...
function setRecordedVolume(a,mix){
  if(!a)return;
  try{a.volume=clamp(masterVolume()*mix*group1Gain(),0,1);}catch(_){ }
}
...
function playRecorded(url,mix,set,manualSelector){
  ...
  a.preload='auto';
  setRecordedVolume(a,mix);
  set.add(a);
  const retire=()=>set.delete(a);
  ...
  if(manualSelector){
    const btn=document.querySelector(manualSelector);
    if(btn){btn.classList.add('fired');setTimeout(()=>btn.classList.remove('fired'),350);}
  }
  return a;
}
function stopSet(set){
  set.forEach(a=>{try{a.pause();a.currentTime=0;}catch(_){ }});
  set.clear();
}
...
function scheduleLight(){
  ...
  lightTimer=setTimeout(()=>{
    if(!lightActive)return;
    playRecorded(LIGHT_URL,.92,lightPlaying,null);
    scheduleLight();
  },rand(300000,600000));
}
function scheduleRocks(first=false){
  ...
  rocksTimer=setTimeout(()=>{
    if(!rocksActive)return;
    playRecorded(ROCK_URL,.95,rockPlaying,null);
    scheduleRocks(false);
  },first?rand(7000,16000):rand(22000,48000));
}

function syncOwnButtons(){
  const light=document.querySelector('[data-audio-layer="light"]');
  if(light){
    light.classList.toggle('active',lightActive);
    light.setAttribute('aria-pressed',String(lightActive));
    const s=light.querySelector('.wa-state');if(s)s.textContent=lightActive?'ON':'OFF';
  }
  const rocks=document.querySelector('[data-audio-layer="rocks"]');
  if(rocks){
    rocks.classList.toggle('active',rocksActive);
    rocks.setAttribute('aria-pressed',String(rocksActive));
    const s=rocks.querySelector('.wa-state');if(s)s.textContent=rocksActive?'ON':'OFF';
  }
}
function syncLive(){
  ...
  if(!live)return;
  const names=[];
  document.querySelectorAll('[data-audio-layer][aria-pressed="true"] b').forEach(b=>{
    const t=(b.textContent||'').trim();if(t&&!names.includes(t))names.push(t);
  });
...
function setLight(on){
  on=!!on;
  if(lightActive===on){syncOwnButtons();syncLive();return;}
  lightActive=on;
  clearLightTimer();
  if(on){
    playRecorded(LIGHT_URL,.92,lightPlaying,null);
    scheduleLight();
  }else stopSet(lightPlaying);
  syncOwnButtons();syncLive();
}
function setRocks(on){
  on=!!on;
  if(rocksActive===on){syncOwnButtons();syncLive();return;}
  rocksActive=on;
  clearRockTimer();
  if(on)scheduleRocks(true);else stopSet(rockPlaying);
  syncOwnButtons();syncLive();
}
function triggerLight(){
  playRecorded(LIGHT_URL,.92,lightPlaying,'[data-audio-trigger="light"]');
  if(lightActive)scheduleLight();
}
function triggerRocks(){
  playRecorded(ROCK_URL,.95,rockPlaying,'[data-audio-trigger="rocks"]');
  if(rocksActive)scheduleRocks(false);
}

function setLayer(id,on){
  if(id==='light')return setLight(on);
  if(id==='rocks')return setRocks(on);
  if(id==='metal')return;
  const out=baseSetLayer&&baseSetLayer(id,on);
  syncLive();
  return out;
}
function trigger(id){
  if(id==='light')return triggerLight();
  if(id==='rocks')return triggerRocks();
  if(id==='metal')return;
...
function install(){
  ...
  const light=document.querySelector('[data-audio-layer="light"]');
  if(light){
    const label=light.querySelector('b');if(label)label.textContent=LIGHT_LABEL;
    const desc=light.querySelector('small');if(desc)desc.textContent='Real failing fluorescent fixture recording — CC0; automatic every 5–10 minutes';
    replaceButton(light,()=>setLight(!lightActive));
  }
  replaceButton(document.querySelector('[data-audio-trigger="light"]'),triggerLight);

  const rocks=document.querySelector('[data-audio-layer="rocks"]');
  if(rocks){
    const label=rocks.querySelector('b');if(label)label.textContent=ROCK_LABEL;
    const desc=rocks.querySelector('small');if(desc)desc.textContent='Real rocks, stones, and gravel falling/rolling — CC0';
    replaceButton(rocks,()=>setRocks(!rocksActive));
  }
  ...
  api.setLayer=setLayer;
  api.trigger=trigger;
  api.startM17=startM17;
  api.stopAll=stopAll;
  api.refreshGroupVolume=refreshGroupVolume;

  const slider=document.getElementById('waVolume');
  if(slider)slider.addEventListener('input',refreshGroupVolume);
...
let tries=0;
const wait=setInterval(()=>{tries++;if(install()||tries>120)clearInterval(wait);},75);
})();