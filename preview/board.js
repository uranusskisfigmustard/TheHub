(() => {
  ...
  const BUILD = '20260919-board-preview-1';
  ...
  const CLASSIFIEDS_GID = '39172266';
  ...
  function render(root, options = {}) {
    ...
    const initialMode = options.mode === 'classifieds' ? 'classifieds' : 'jobs';
    ...
    root.innerHTML = `
      <div class="board-preview-heading">
        ...
        <h1>${initialMode === 'classifieds' ? 'Classifieds' : 'Contracts'}</h1>
      ...
      <div class="board-statusbar">
        <span data-board-status>LOADING BOARDS…</span>
        <span data-board-transport>QUALIFICATION TRANSPORT // CHECKING</span>
    ...
    function loadSheet(gid, callbackName, timeoutMs = 9000) {
      return new Promise((resolve, reject) => {
        const callback = `__hubBoardPreview_${callbackName}_${Date.now()}_${Math.random().toString(36).slice(2)}`;
        ...
        window[callback] = response => {
          try {
            if (!response || response.status === 'error') throw new Error(response?.errors?.[0]?.message || 'Google Sheet returned an error.');
        ...
        script.src = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?gid=${gid}&headers=1&tqx=${tqx}&_=${cacheBust}`;
        script.onerror = () => finish(reject, new Error('Google Sheets listing feed unavailable.'));
        document.body.appendChild(script);
        timer = setTimeout(() => finish(reject, new Error('Listing feed timed out.')), timeoutMs);
      });
    }
    ...
    function configureFilters() {
      if (state.mode === 'jobs') {
        ...
        qualificationFilter.hidden = false;
      } else {
        ...
        qualificationFilter.hidden = true;
      }
    }
    ...
    function renderJobs() {
      ...
      const filtered = state.jobs.filter(job => {
        ...
        const qualificationMatch = state.qualificationFilter === 'all' || (state.qualificationFilter === 'ready' && qs === 'ready') || (state.qualificationFilter === 'action' && qs !== 'ready');
        return (!q || hay.includes(q)) && (!workType || String(job['Work Type'] || '') === workType) && (!goodFit || fits.includes(goodFit)) && qualificationMatch;
      });
      ...
      cards.innerHTML = filtered.map(job => {
        ...
        return `<article class="board-card contract-card"><div class="board-title">${esc(job.Title)}</div><div class="board-pay">${esc(job.Pay)}</div><div class="board-meta">${esc(job.Employer)}${job['Work Type'] ? ` · ${esc(job['Work Type'])}` : ''}</div>${qualificationBox(job)}<div class="board-summary">${esc(job.Summary)}</div>${details ? `<details class="board-details"><summary>View Details</summary><div>${esc(details)}</div></details>` : ''}${betweenSessionBlock(job)}${jobId ? '<button class="board-disabled-action" type="button" disabled>PREVIEW // ACCEPT CONTRACT DISABLED</button>' : ''}</article>`;
      }).join('');
    }
    ...
    function renderClassifieds() {
      ...
      const filtered = posted.filter(row => {
        ...
        return (!q || hay.includes(q)) && (!category || String(row.Category || '') === category) && (!listingType || (listingType === 'wanted' ? wanted : !wanted));
      });
    ...
    function renderCurrent() {
      heading.textContent = state.mode === 'jobs' ? 'Contracts' : 'Classifieds';
      subtitle.textContent = state.mode === 'jobs' ? 'Independent work listings — review terms before acceptance.' : 'Secondhand goods, surplus equipment, lease transfers, and things somebody wants gone.';
      footer.textContent = state.mode === 'jobs' ? 'GOOD FIT is advisory. PREFERRED SKILLS strengthen a bid but are not mandatory. REQUIRED SKILLS and formal qualification terms are hard gates where shown.' : 'Listings are seller-supplied. Condition, compatibility, title, transport, installation, certification, and facility approval remain the buyer’s responsibility unless the listing says otherwise.';
    ...
    async function refreshEligibility() {
      ...
      if (!jobIds.length) {
        state.eligibility = {}; state.eligibilityValid = true;
        transport.textContent = 'QUALIFICATION TRANSPORT // POST COMPATIBILITY'; renderCurrent(); return;
      }
      try {
        const response = await postRequest('preflight', { jobs: jobIds });
        if (!response || response.ok === false || !response.states || typeof response.states !== 'object') throw new Error(response?.error || 'Qualification preflight unavailable.');
        state.eligibility = response.states;
        state.eligibilityValid = true;
        transport.textContent = 'QUALIFICATION TRANSPORT // POST COMPATIBILITY';
      } catch (_) {
        state.eligibility = {};
        state.eligibilityValid = false;
        transport.textContent = 'QUALIFICATION TRANSPORT // UNAVAILABLE';
      }
      renderCurrent();
    }
    ...
    async function loadJobs() {
      ...
      try {
        const rows = await loadSheet(JOBS_GID, 'jobs');
        ...
        state.jobsLoaded = true;
        state.jobsSource = cached.rows.length ? `CACHED / STALE // Contracts last loaded: ${new Date(cached.time).toLocaleString()}` : `CONTRACT DATA UNAVAILABLE // ${String(error?.message || error)}`;
      }
      configureFilters();
      renderCurrent();
      refreshEligibility();
    ...
    async function loadClassifieds() {
      state.classifiedsLoaded = false;
      state.classifiedsSource = 'LOADING CLASSIFIEDS…';
      try {
        const rows = await loadSheet(CLASSIFIEDS_GID, 'classifieds');
        state.classifieds = rows.sort((a, b) => Number(a['Sort Order'] || 9999) - Number(b['Sort Order'] || 9999));
        state.classifiedsLoaded = true;
        ...
        state.classifiedsLoaded = true;
        state.classifiedsSource = cached.rows.length ? `CACHED / STALE // Classifieds last loaded: ${new Date(cached.time).toLocaleString()}` : `CLASSIFIED DATA UNAVAILABLE // ${String(error?.message || error)}`;
      }
      configureFilters();
      renderCurrent();
    }
    ...
    async function refreshAll() {
      ...
      await Promise.all([loadJobs(), loadClassifieds()]);
      refreshButton.disabled = false;
    }

    [search, primary, secondary].forEach(element => element.addEventListener('input', renderCurrent));
    clearButton.addEventListener('click', () => {
      search.value = ''; primary.value = ''; secondary.value = ''; state.qualificationFilter = 'all';
      qualificationFilter.querySelectorAll('[data-qf]').forEach(button => button.classList.toggle('active', button.dataset.qf === 'all'));
      renderCurrent();
    });
