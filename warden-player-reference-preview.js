...
const SECTIONS = [
  ...
  {
    group:'IMMEDIATE',
    title:'Professional Qualifications & Access',
    items:[
  ...
  {
    group:'IMMEDIATE',
    title:'Restorative Prosthetics — Grades 0–3 Only',
    items:[
...
function installStyles(){
  ...
  s.textContent=`
    ...
    .wc-ref-note{max-width:760px;color:var(--muted);font-size:.78rem}
    .wc-ref-jump{display:flex;gap:6px;flex-wrap:wrap}
    .wc-ref-jump button{border:1px solid var(--line);background:#15191c;color:var(--text);padding:6px 9px;font:inherit;font-size:.7rem;font-weight:800;letter-spacing:.05em;text-transform:uppercase}
    .wc-ref-jump button:hover{border-color:var(--accent)}
    .wc-ref-group-title{margin:18px 0 8px;padding:7px 9px;border-left:3px solid var(--accent);background:#15191c;color:var(--text);font-size:.8rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase}
    .wc-ref-group-title.next{border-left-color:#6f8290;color:#bcc6cc}
    .wc-ref-section{border:1px solid var(--line);background:var(--panel2);margin:0 0 10px;padding:12px}
    .wc-ref-section h3{margin:0 0 9px;font-size:.9rem;letter-spacing:.055em;text-transform:uppercase}
    .wc-ref-item{border-top:1px solid #303538;padding:9px 0}
    .wc-ref-item:first-of-type{border-top:0;padding-top:0}
    ...
    .wc-ref-next-note{margin:4px 0 12px;padding:9px 10px;border:1px solid #46515a;background:#15191c;color:#b7c1c7;font-size:.75rem}
    @media(max-width:760px){
      .wc-ref-section{padding:10px}
      .wc-ref-head{gap:10px}
...
function renderSections(){
  ...
  return SECTIONS.map(section=>{
    ...
    if(section.group!==lastGroup){
      ...
      group=`<div id="wcRef${section.group}" class="wc-ref-group-title ${section.group==='NEXT'?'next':''}">${section.group==='IMMEDIATE'?'Immediate — Proposed Initial Player Reference':'Next — Hold Until the Choice Becomes Active'}</div>`;
      if(section.group==='NEXT') group += '<div class="wc-ref-next-note">These entries are not proposed for the initial player release. They are queued so the wording is ready when the corresponding choice becomes active.</div>';
    }
    const items=section.items.map(([title,paras])=>`<div class="wc-ref-item"><h4>${esc(title)}</h4>${paras.map(p=>`<p>${p}</p>`).join('')}</div>`).join('');
    return `${group}<section class="wc-ref-section"><h3>${esc(section.title)}</h3>${items}</section>`;
  }).join('');
}
  ...
  panel.innerHTML=`
    <div class="wc-ref-head">
      <div>
        ...
        <h2>Player Reference — Immediate & Next</h2>
        <div class="wc-ref-note">Proposed player-facing wording. Nothing on this screen is exposed through the player console. Approve or revise here before a separate player-side publication.</div>
      </div>
      <div class="wc-ref-jump">
        <button type="button" data-ref-jump="IMMEDIATE">Immediate</button>
        <button type="button" data-ref-jump="NEXT">Next</button>
      </div>
    </div>
    ${renderSections()}`;
  consoleEl.appendChild(panel);
  panel.querySelectorAll('[data-ref-jump]').forEach(btn=>btn.addEventListener('click',()=>{
    $(`wcRef${btn.dataset.refJump}`)?.scrollIntoView({behavior:'smooth',block:'start'});
  }));
}
...
function syncVisibility(){
  ...
  if(!unlocked && $('playerReferencePreview')) $('playerReferencePreview').classList.add('wc-workspace-hidden');
}
