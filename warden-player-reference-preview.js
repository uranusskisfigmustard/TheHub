...
function renderCategory(category){
  ...
  const nextBlock=next.length?`<div class="wc-ref-next-divider">Next — Hold Until the Choice Becomes Active</div><div class="wc-ref-next-note">These entries are queued for later player publication and are not part of the initial release.</div>${next.map(renderSection).join('')}`:'';
...
function selectCategory(panel,key){
  panel.querySelectorAll('[data-ref-category]').forEach(el=>el.classList.toggle('hidden',el.dataset.refCategory!==key));
...
function installPanel(){
  ...
  panel.innerHTML=`
    <div class="wc-ref-head">
      <div>
        ...
        <div class="wc-ref-note">Proposed player-facing wording. Nothing on this screen is exposed through the player console. Campaign Rules apply wherever the PCs are. RP / In-World Rules cover systems the characters encounter through play.</div>
      </div>
    </div>
  ...
  panel.querySelectorAll('[data-ref-tab]').forEach(btn=>btn.addEventListener('click',()=>selectCategory(panel,btn.dataset.refTab)));
