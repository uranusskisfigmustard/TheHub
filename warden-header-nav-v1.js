...
const WORKSPACE_GROUP={
  dashboard:'dashboard',
  contracts:'operations',session:'operations',
  npcs:'world',factions:'world',
  progression:'campaign',reference:'campaign',
  admin:'admin'
};
const WORKSPACE_LABEL={
  dashboard:'Dashboard',contracts:'Contracts',session:'Session',npcs:'NPCs',factions:'Factions',progression:'Progression',reference:'Player Reference',admin:'Admin'
};
let queued=false;
...
function installStyles(){
  ...
  style.textContent=`
    ...
    .wc-nav-primary{flex-wrap:wrap}
    .wc-nav-secondary{margin-top:7px;flex-wrap:wrap}
    ...
    .wc-primary-btn{padding:8px 14px;font-size:.72rem;letter-spacing:.08em}
    .wc-secondary-btn{padding:7px 11px;font-size:.7rem;letter-spacing:.06em}
    ...
    @media(max-width:760px){
      ...
      .wc-nav-secondary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;width:100%;margin-top:5px}
      .wc-nav-secondary:empty{display:none}
    ...
    @media(max-width:390px){
      .wc-primary-btn{font-size:.63rem;padding-left:3px;padding-right:3px}
      .wc-secondary-btn{font-size:.62rem}
    }
  `;
  document.head.appendChild(style);
}

function sourceButtons(nav){
  return [...nav.querySelectorAll(':scope > .wc-workspace-btn[data-workspace]')];
}

function sourceFor(nav,key){
  return nav.querySelector(`:scope > .wc-workspace-btn[data-workspace="${key}"]`);
}

function activeWorkspace(nav){
  return sourceButtons(nav).find(button=>button.classList.contains('active'))?.dataset.workspace || 'dashboard';
}

function ensureHosts(nav){
  let primary=nav.querySelector(':scope > .wc-nav-primary');
  let secondary=nav.querySelector(':scope > .wc-nav-secondary');
  if(!primary){
    primary=document.createElement('div');
    primary.className='wc-nav-primary';
    primary.setAttribute('aria-label','Warden console sections');
    nav.insertBefore(primary,nav.firstChild);
  }
  if(!secondary){
    secondary=document.createElement('div');
    secondary.className='wc-nav-secondary';
    secondary.setAttribute('aria-label','Warden console workspaces');
    primary.insertAdjacentElement('afterend',secondary);
  }
  return {primary,secondary};
}

function activateWorkspace(nav,key){
  const source=sourceFor(nav,key);
...
function renderPrimary(nav,primary,activeGroup){
  GROUPS.forEach(group=>{
    ...
    if(!button){
      ...
      button.type='button';
      button.className='wc-primary-btn';
      button.dataset.wcGroup=group.key;
      button.textContent=group.label;
      button.addEventListener('click',()=>{
        const current=WORKSPACE_GROUP[activeWorkspace(nav)]||'dashboard';
        if(current===group.key)return;
        activateWorkspace(nav,group.defaultWorkspace);
      });
...
function syncCount(proxy,source){
  const sourceCount=source?.querySelector('.wc-count');
  let proxyCount=proxy.querySelector('.wc-count');
  if(!sourceCount){proxyCount?.remove();return;}
  if(!proxyCount){
    proxyCount=document.createElement('span');
    proxyCount.className='wc-count';
    proxy.appendChild(proxyCount);
  }
  proxyCount.textContent=sourceCount.textContent||'0';
}

function renderSecondary(nav,secondary,workspace,activeGroup){
  const keys=ORDER.filter(key=>WORKSPACE_GROUP[key]===activeGroup && !['dashboard','admin'].includes(key) && sourceFor(nav,key));
  ...
  keys.forEach(key=>{
    ...
    if(!button){
      button=document.createElement('button');
      button.type='button';
      button.className='wc-secondary-btn';
      button.dataset.wcWorkspace=key;
      const label=document.createElement('span');
      label.className='wc-secondary-label';
      label.textContent=WORKSPACE_LABEL[key]||key;
      button.appendChild(label);
      button.addEventListener('click',()=>activateWorkspace(nav,key));
      secondary.appendChild(button);
    }
    syncCount(button,sourceFor(nav,key));
...
function normalize(){
  ...
  nav.classList.add('wc-tiered-nav');
  const sources=sourceButtons(nav);
  sources.forEach(button=>button.classList.add('wc-nav-source'));

  const ordered=ORDER.map(key=>sourceFor(nav,key)).filter(Boolean);
  const current=sources.filter(button=>ORDER.includes(button.dataset.workspace));
  ...
  const workspace=activeWorkspace(nav);
  const activeGroup=WORKSPACE_GROUP[workspace]||'dashboard';
  const {primary,secondary}=ensureHosts(nav);
  renderPrimary(nav,primary,activeGroup);
  renderSecondary(nav,secondary,workspace,activeGroup);
  ...
  if(nav.dataset.canonicalHeaderWatch!=='1'){
    ...
    new MutationObserver(mutations=>{
      const externalMutation=mutations.some(m=>{
        ...
        return !target?.closest?.('.wc-nav-primary,.wc-nav-secondary');
      });
