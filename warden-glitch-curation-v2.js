...
const APPROVED_POOL=[
  ...
  ['candidate','roll','ROLLING INTERFERENCE BAND','A thinner, softer moving interference band; related to Scanline Burst but less forceful.'],
...
function installStyles(){
  ...
  const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
    ...
    #${PANEL_ID} .wgv2-head{margin:17px 0 8px;color:var(--accent);font-size:.74rem;font-weight:800;letter-spacing:.09em;text-transform:uppercase}
    ...
    #${PANEL_ID} .wgv2-card .btn{width:100%;text-align:left;padding:7px 9px;font-size:.72rem;border-color:var(--ok)}
...
function install(){
  ...
  p.innerHTML=`
    ...
    <div class="wgv2-note">WARDEN-ONLY evaluation panel. Eleven approved effects remain in the automatic pool, ordered left-to-right from most recommended to least recommended. Approved timing rules for eventual player use remain unchanged: irregular spacing, usually 2–4 events per 5 minutes, normally at least 30–45 seconds apart, occasional 2–3 minute quiet periods, and per-effect cooldowns to prevent obvious repetition.</div>
    <div class="wgv2-head">Approved</div>
    <div class="wgv2-grid">${APPROVED_POOL.map(card).join('')}</div>
